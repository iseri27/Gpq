...
num_gpus = 2
batch_size = 4
num_iters_per_epoch = 28130 // (batch_size * num_gpus)
num_epochs = 24

pruning_interval = num_iters_per_epoch * 6 // num_pruned
...
model = dict(
    ...
    img_neck=dict(
        type="CPFPN", in_channels=[1024, 2048], out_channels=embed_dims, num_outs=2
    ),
    pts_bbox_head=dict(
        type="PETRGpqHead",
        ...
        positional_encoding=dict(
            type="SinePositionalEncoding3D", num_feats=embed_dims // 2, normalize=True
        ),
        loss_cls=dict(
            type="FocalLoss",
            use_sigmoid=True,
