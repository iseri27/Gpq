...
class StreamPETRGpqHead(AnchorFreeHead):
    ...
    Args:
        ...
        num_reg_fcs (int, optional):
            Number of fully-connected layers used in `FFN`,
            which is then used for the regression head. Default 2.
        transformer (obj:`mmcv.ConfigDict`|dict):
            Config for transformer. Default: None.
        ...
        loss_cls (obj:`mmcv.ConfigDict`|dict):
            Config of the classification loss. Default `CrossEntropyLoss`.
        loss_bbox (obj:`mmcv.ConfigDict`|dict):
            Config of the regression loss. Default `L1Loss`.
        tran_cfg (obj:`mmcv.ConfigDict`|dict):
            Training config of transformer head.
        test_cfg (obj:`mmcv.ConfigDict`|dict):
            Testing config of transformer head.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if class_weight is not None and (self.__class__ is StreamPETRGpqHead):
        ...
        super(StreamPETRGpqHead, self).__init__(
        ...
        if self.num_propagated > 0:
            if not self.test_mode:
                pseudo_reference_points_ = torch.stack(
                    [
                        point
                        for point in self.pseudo_reference_points[: self.num_propagated]
                    ]
                )
            else:
                pseudo_reference_points_ = self.pseudo_reference_points.weight[
                    : self.num_propagated, ...
                ]
    ...
    def _load_from_state_dict(
        ...
        if (version is None or version < 2) and self.__class__ is StreamPETRGpqHead:
            convert_dict = {
        ...
        for key_ in state_dict:
            ...
            if key_.startswith("pts_bbox_head.pseudo_reference_points"):
                i = key_.split(".")[-1].strip()
                if i.isdigit() and int(i) >= self.num_propagated:
                    deprecated_keys.append(key_)
        ...
        if self.test_mode:
            ...
            for i in range(self.num_query):
                reference_points.append(
                    state_dict.pop(f"pts_bbox_head.reference_points.{i}")
                )
            pseudo_reference_points = []
            for i in range(self.num_propagated):
                pseudo_reference_points.append(
                    state_dict.pop(f"pts_bbox_head.pseudo_reference_points.{i}")
                )

            reference_points_weight = torch.stack(reference_points)
            pseudo_reference_points_weight = torch.stack(pseudo_reference_points)
            state_dict["pts_bbox_head.reference_points.weight"] = (
                reference_points_weight
            )
            state_dict["pts_bbox_head.pseudo_reference_points.weight"] = (
                pseudo_reference_points_weight
            )
    ...
    def forward(self, memory_center, img_metas, topk_indexes=None, **data):
        ...
        # start_layer = 0 if not self.test_mode else outs_dec.shape[0] - 1

        # for lvl in range(start_layer, outs_dec.shape[0]):
    ...
    def _get_target_single(self, cls_score, bbox_pred, gt_labels, gt_bboxes):
        ...
        if a bbox's shape is [..., 10], it is in normalized coordinate:
            (cx, cy, log(dx), log(dy), cz, log(dz), sin(rot), cos(rot), vx, vy).
        else if its shape is [..., 9], its coordinate is:
            (cx, cy, cz, dx, dy, dz, rot, vx, vy).
        Args:
            cls_score (Tensor):
                Box score logits from a single decoder layer for one image.
                Shape [num_query, cls_out_channels].
            bbox_pred (Tensor):
                Sigmoid outputs from a single decoder layer for one image,
                with shape [num_query, 10] in normalized coordinate.
            gt_bboxes (Tensor):
                Ground truth bboxes for one image with shape (num_gts, 10).
            gt_labels (Tensor):
                Ground truth class indexes for one image with shape (num_gts, ).
        Returns:
    ...
    def get_targets(
        ...
        Args:
            ...
            bbox_preds_list (list[Tensor]):
                Sigmoid outputs from a single decoder layer for each image,
                with normalized coordinate and shape [num_query, 10].
            gt_bboxes_list (list[Tensor]):
                Ground truth bboxes for each image with shape (num_gts, 10).
            gt_labels_list (list[Tensor]):
                Ground truth class indexes for each image with shape (num_gts, ).
        Returns:
    ...
    def loss_single(
        ...
        Args:
            cls_scores (Tensor):
                Box score logits from a single decoder layer for all images.
                Shape [bs, num_query, cls_out_channels].
            bbox_preds (Tensor):
                Sigmoid outputs from a single decoder layer for all images,
                with normalized coordinate and shape [bs, num_query, 10].
            gt_bboxes_list (list[Tensor]):
                Ground truth bboxes for each image with shape (num_gts, 10).
            gt_labels_list (list[Tensor]):
                Ground truth class indexes for each image with shape (num_gts, ).
    ...
    def loss(self, gt_bboxes_list, gt_labels_list, preds_dicts):
        ...
        Args:
            gt_bboxes_list (list[Tensor]):
                Ground truth bboxes for each image with shape (num_gts, 10)
                in [cx, cy, log(dx), log(dy), cz, log(dz), sin(rot), cos(rot), vx, vy] format.
            gt_labels_list (list[Tensor]):
                Ground truth class indexes for each image with shape (num_gts, ).
            preds_dicts:
                all_cls_scores (Tensor):
                    Classification score of all decoder layers, has shape
                    [nb_dec, bs, num_query, cls_out_channels].
                all_bbox_preds (Tensor):
                    Sigmoid regression outputs of all decode layers.
                    Each is a 9D-tensor with shape [num_dec, bs, num_query, 9]
                    unnormalized coordinate format
                    (cx, cy, cz, dx, dy, dz, rot, vx, vy).
